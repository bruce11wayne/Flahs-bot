...
let handler = async (message, { conn, text, usedPrefix, command }) => {
  if (!text) {
    return conn.reply(message.chat, "[💫] *ما هي الصور التي تريدها*", message);
  }
  ...
  for (let imageUrl of selectedImages) {
    results.push({
      ...
      'footer': proto.Message.InteractiveMessage.Footer.fromObject({
        'text': "by bruce" // ضع العلامة المائية هنا
      }),
      ...
      'nativeFlowMessage': proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
        'buttons': [{
          'name': "cta_url",
          'buttonParamsJson': "{\"display_text\":\"「⛈️ ╎بينتر╎⛈️ 」\",\"Url\":\"https://chat.whatsapp.com/Ilr00RrPBu95bxkxQ5rZbs" + text + "\",\"merchant_url\":\"https://chat.whatsapp.com/Ilr00RrPBu95bxkxQ5rZbs" + text + "\"}",  
        }]
      })
    });
  }
